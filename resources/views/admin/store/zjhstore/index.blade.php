...
@section('content')
    <div class="layui-tab layui-tab-card">
        ...
        <ul class="layui-tab-title">
            ...
            <li  class=" {{$key=='zjh_junior'?'layui-this':''}}" value="{{$key or ''}}" onclick="change(this)">{{$val or ''}}</li>
            @endforeach
            @endif
        </ul>
        <div class="layui-tab-content" style="height: 100%;">
            ...
            <div class="layui-tab-item {{$key=='zjh_junior'?'layui-show':''}}" id="{{$key or ''}}" name="{{$key or ''}}">
        ...
        layui.use(['layer','table','form'],function () {
            ...
            $("#save").click(function () {
               ...
               // var data = JSON.stringify(data);
                $.post("{{route('admin.zjhstore.save')}}", {data: data, list: gameList}, function (res) {
                    if (res.code == 0){
            ...
            //发送到服务器配置
            $("#send").click(function(){
                $.post("{{route('admin.zjhstore.send')}}",{}, function (res) {
                    if (res.code == 0){
                        layer.msg(res.msg,{icon:1})
                    }else {
                        layer.msg(res.msg,{icon:2})
