...
@section('content')
    <div class="layui-tab layui-tab-card">
        ...
        <ul class="layui-tab-title">
            ...
            <li  class=" {{$key=='lfdj_normal'?'layui-this':''}}" value="{{$key or ''}}" onclick="change(this)">{{$val or ''}}</li>
            @endforeach
            @endif
        </ul>
        <div class="layui-tab-content" style="height: 100%;">
            ...
            <div class="layui-tab-item {{$key=='lfdj_normal'?'layui-show':''}}" id="{{$key or ''}}" name="{{$key or ''}}">
                        ...
                        <tr id="cattle_award">
                            ...
                            <td>
                                ...
                                <div class="layui-form-item">
                                    ...
                                    <div class="layui-input-inline">
                                        <input type="number" id="special_rate" name="lose_limit" value="{{$val['special_rate'] or ''}}" lay-verify="required" placeholder="0" class="layui-input" >
                                    </div>
                                </div>
                            </td>
                        </tr>
        ...
        layui.use(['layer','table','form'],function () {
            ...
            $("#save").click(function () {
               ...
               // var data = JSON.stringify(data);
                $.post("{{route('admin.lfdjstore.save')}}", {data: data, list: gameList}, function (res) {
                    if (res.code == 0){
            ...
            //发送到服务器配置
            $("#send").click(function(){
                $.post("{{route('admin.lfdjstore.send')}}",{}, function (res) {
                    if (res.code == 0){
                        layer.msg(res.msg,{icon:1})
                    }else {
                        layer.msg(res.msg,{icon:2})
