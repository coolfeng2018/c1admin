            ...
            <script type="text/html" id="options">
                <div class="layui-btn-group">
                    {{--@can('operate.bankorder.refuse')--}}
                        <a class="layui-btn layui-btn-danger layui-btn-sm" lay-event="detail">记录</a>
                    {{--@endcan--}}
                </div>
            </script>
        </div>
        ...
        <script>
            layui.use(['layer','table','laydate'],function () {
                ...
                var dataTable = table.render({
                    elem: '#dataTable'
                    ,url: "{{ route('admin.fish.data') }}" //数据接口
                    ,cellMinWidth: 80
                    ,page: true //开启分页
                    ,cols: [[ //表头
                        //{checkbox: true,fixed: true}
                        {field: 'userName', title: '玩家账号', sort: true, align: 'center'}
                        ,{field: 'uid', title: '玩家ID', align: 'center'}
                        ,{field: 'bullet_count', title: '子弹发送总量', align: 'center'}
                        ,{field: 'bullet_coins', title: '子弹价值', align: 'center'}
                        ,{field: 'fish_coins', title: '鱼总价值', align: 'center'}
                        ,{field: 'money', title: '收支', align: 'center'}
                    ...
                    var data = obj.data                      //获得当前行数据
                        ...
                        if (layEvent == 'detail'){
                            layer.open({
                                ...
                                area: ['60%', '50%'], //宽高
                                content: "fish/detail?uid="+uid+'&userName='+userName,//+'&stime='+stime+'&etime='+etime, //弹出的页面
                ...
                $("#search").click(function () {
                    ...
                    var etime = $("#etime").val();
                    dataTable.reload({
                        where:{uid:uid,userName:userName,stime:stime,etime:etime},
                        page:{curr:1}
                    })
                });
