        ...
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-form" >
                <div class="layui-input-inline hidden">
                    <input type="hidden" name="table_list" id="table_list" value="{{$tableListArr}}" class="layui-input">
    ...
    @can('operate.stock')
        <script>
            layui.use(['laydate','table'],function () {
                ...
                var tableListArr = JSON.parse($("#table_list").val());
                ...
                var cols = [{field: 'today', title: '日期', align: 'center'}];
                $.each(tableListArr,function (index,value) {
                    cols.push({field: prefix+index, title: value, align: 'center'});
                });
                cols.push({field:'totle', title: '统计', fixed: 'right'});

                //用户表格初始化
                var dataTable = table.render({
                    elem: '#dataTable'
                    ,url: "{{ route('admin.stock.data') }}" //数据接口
                    ,cellMinWidth: 110
                    ,page: false //开启分页
                    ,cols: [cols]
                });
