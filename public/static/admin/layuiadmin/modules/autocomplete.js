layui.define(['jquery', 'laytpl', 'layer'], function (e) {
    "use strict";
    layui.link('/static/admin/layuiadmin/style/autocomplete.css');
    var hint = layui.hint(),
        $ = layui.jquery,
        laytpl = layui.laytpl,
        layer = layui.layer,
    ...
    job.prototype.events = function () {
        ...
        $(document).on('click', function (e) {
            ...
            if (_e !== undefined) {
                ...
                var _index = $(_target.parentNode).data('index');
                _elem.val(laytpl(_config.template_val).render(_config.data[_self.index][_index])), _config.onselect == undefined || _config.onselect(_config.data[_self.index][_index])
            }
